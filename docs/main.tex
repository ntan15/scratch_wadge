...
\label{eq:nonlineqs}
\end{equation}
where $\bm{u}(\bm{x},t)$ denote the \emph{conservative variables} for this system.  %We assume the system is well-posed under appropriate boundary conditions.
...
We assume that the mesh is quasi-uniform.  The mesh is also assumed to be watertight, such that normals are consistent across neighboring elements as follows: for a shared face $f$ between $\bm{D}^k$ and $\bm{D}^{k,+}$, the scaled outward normal vectors for each element are equal and opposite at all points such that 
\begin{align}
...
%\end{lemma}
%\begin{proof}
%The result is the consequence of a scaling argument \cite{brenner2007mathematical} %and a Rayleigh quotient bound involving the largest eigenvalue of the generalized eigenvalue problem.  The scaling argument yields that
%\begin{align}
%\nor{v}_{W^{N+1,2}\LRp{D^k}} = C_1 h^{-(N+1)} \nor{v}_{W^{N+1,2}\LRp{\hat{D}}}.
%\end{align}
%The quantity $\nor{v}_{W^{N+1,2}\LRp{\hat{D}}}$ can be bounded by noting that $v\in P^N$.  Since $P^N$ is finite-dimensional, the Sobolev norm can be bounded from above by the $L^2$ norm of $\hat{D}$ (with a constant $C_{2}$ depending on $N, d$.  Then, by another scaling argument, we have 
%\begin{align}
%\nor{v}_{W^{N+1,2}\LRp{\hat{D}}} \leq C_{2} \nor{v}_{L^2\LRp{\hat{D}}} \leq %C_{2} h \nor{v}_{L^2\LRp{D^k}}.
%\end{align}
...
%\begin{align*}
%\nor{\Pi_N^k  u - P_N^k  u}_{L^2\LRp{D^k}}^2 &= \LRb{\LRp{\Pi_N^k u,vJ}_{\hat{D}} - \LRp{P_N^k  u,vJ}_{\hat{D}}}\\
...
The local $L^2$ error $\nor{{J^k\bm{G}^k_{ij}}-\tilde{J^k\bm{G}^k_{ij}}}_{L^2\LRp{D^k}}$ can be bounded as follows:
\begin{align*}
\nor{{J^k\bm{G}^k_{ij}}-\tilde{J^k\bm{G}^k_{ij}}}_{L^2\LRp{D^k}} &\leq \nor{\sqrt{J^k}}_{L^2\LRp{\hat{D}}} \nor{\hat{\Grad}\times \LRp{\bm{r}-I_{N+1}\bm{r}}}_{L^2\LRp{\hat{D}}}\\
&\leq C_1 \nor{\sqrt{J^k}}_{L^2\LRp{\hat{D}}} \LRb{\LRp{\bm{r}-I_{N+1}\bm{r}}}_{H^1\LRp{\hat{D}}} \leq \tilde{C}_N \nor{\sqrt{J^k}}_{L^2\LRp{\hat{D}}} \LRb{\bm{r}}_{W^{N+2,2}\LRp{\hat{D}}},
\end{align*}
...
\begin{align}
\LRb{\bm{r}}_{W^{N+2,2}\LRp{\hat{D}}} \leq C_2 h^{N+2}\nor{\bm{r}}_{W^{N+2,2}\LRp{D^k}}.
\label{eq:localgeoerr}
\end{align}
The global error estimate results from squaring (\ref{eq:localgeoerr}), noting that $\sum_k \nor{\sqrt{J^k}}^2_{L^2\LRp{\hat{D}}} = \LRb{\Omega}$, and summing over all elements.
%\[
...
Next, we verify the semi-discrete entropy conservation and accuracy of the proposed high order methods for the compressible Euler equations on curved meshes in two and three dimensions.  
...
\begin{equation}
p_{\rm avg} = \frac{\avg{\rho}}{2\avg{\beta}}, \qquad p_{\rm avg}^{\log} = \frac{\avg{\rho}^{\log}}{2\avg{\beta}^{\log}}, \qquad \nor{\bm{u}}^2_{\rm avg} = 2(\avg{u}^2 + \avg{v}^2) - \LRp{\avg{u^2} +\avg{v^2}}.  
\end{equation}

We begin by testing the propagation of a shock on a two-dimensional curved mesh using a discontinuous profile.  
\begin{figure}
\centering
...
\caption{\note{Finish}.  In both cases, the magnitude of the right hand side is $O\LRp{10^{-14}}$, indicating that  }
