...
\end{pmatrix}
\]
In this problem, we take $x_0 = y_0 = 5$, $p_0 = {1}/{\gamma}$, and $\Pi_{\max} = .4$.  We solve the problem on the domain $[0,10]\times [0,20]\times [0,10]$.  
\begin{figure}
\centering
...
\label{fig:mesh3d}
\end{figure}
We use \textsc{GMSH} to construct three unstructured meshes consisting of 1354, 9543, and 72923 tetrahedra.  The latter two meshes are shown in Figure~\ref{fig:mesh3d}.  We apply a curvilinear warping by again mapping nodal positions each tetrahedron to warped nodal positions $(\tilde{x},\tilde{y},\tilde{z})$ via
\begin{align*}
...
\end{loglogaxis}
\end{tikzpicture}
\caption{Convergence of $L^2$ errors for the 3D isentropic vortex problem on affine and curved meshes, with optimal $O(h^{N+1})$ rates of convergence shown for reference.}
...
Our last numerical experiment investigates the behavior of ESDG schemes for the inviscid Taylor-Green vortex \cite{ae1937mechanism, gassner2016split, crean2018entropy}.  
