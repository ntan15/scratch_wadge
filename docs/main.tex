...
Entropy stable methods have largely relied on a finite difference summation-by-parts framework.  The summation-by-parts property holds for spectral element DG methods (DG-SEM), which assume a polynomial basis which collocates the solution at Gauss-Legendre-Lobatto (GLL) quadrature nodes on tensor product elements.  This approach is difficult to replicate on simplicial elements, as analogous quadrature rules on the triangle and tetrahedron contain more nodes than the dimension of the underlying polynomial space \cite{hicken2016multidimensional}.  As a result, the entropy stable schemes introduced in \cite{chen2017entropy, crean2018entropy} are not ``modal'', or associated with an underlying basis or approximation space.  

Entropy stable schemes were extended to modal discretizations  in \cite{chan2017discretely}, allowing for the use of arbitrary basis functions and over-integrated quadrature rules.  In this work, we show how to extend the construction of high order entropy stable schemes to curvilinear meshes.  \note{finish}
...
%\lcwnote{(Is this true when have a non-affine element mapping?)} \note{No - good point.  I specified affine meshes in the revised text}.  
...
%\lcwnote{(It seems to be positive definite due to the fact we have a positive weight quadrature, not Assumption~\ref{ass:quad}.)} \note{Yes, but the degree of exactness of the quadrature is also necessary or it can be only positive semi-definite.  For example, if you just use a degree $2N$ quadrature, I think you can end up with a singular mass matrix.}; \lcwnote{Cool, I see this now.}
...
%\lcwnote{(What about $\bm{J}^{\bm{G}^k_{ij}}$ instead of $\bm{JG}^k_{ij}$ to make it look more like a single object? I am debating if I should revert the changes to add the Hadamard product to the other surface terms.)} \note{Hm...I'm not sure about the expanding superscripts/subscripts.  What if we changed it to a different name instead of $J\bm{G}_{ij}$, like $\tilde{\bm{G}}^k_{ij}$?} \lcwnote{I got used to the way you have it, I don't think we need to change.}
...
\end{align*} 
Additionally, we approximate the geometric terms using (\ref{eq:iconscurl2}) to ensure that the geometric terms obey the discrete GCL.  
...
Figure~\ref{fig:converge3d} shows $L^2$ errors for $N = 2,3,4$ on both affine and curved meshes.  In both cases, we observe optimal  $O(h^{N+1})$ asymptotic rates of convergence for $N = 2,3$, while for $N = 4$ we observe a rate which is slightly higher than $O(h^{N+1/2})$.  This may be due to the fact that the time-stepper is $4$th order while the spatial discretization is $5$th order.  
...
\]
as well as the kinetic energy dissipation rate $-\pd{\kappa}{t}$, which we approximate by differencing $\kappa(t)$.  Figure~\ref{fig:tg} shows the evolution of $\kappa(t)$ over time for both affine and curvilinear meshes with $h = \pi/8$.  
