...
A key property of the matrix $\bm{D}_N^i$ is that it satisfies a summation-by-parts property.  Let $\bm{Q}_N^i$ be the scaling of $\bm{D}_N^i$ by the diagonal matrix of volume and surface quadrature weights 
\[
...
\begin{equation}
\sum_{j=1}^d\pd{}{\hat{x}_j}J^k\bm{G}^k_{ji} = 0,
\label{eq:gcl}
\end{equation}
or that $\hat{\Grad}\cdot \LRp{J^k\bm{G}^k}^T = 0$.  Using (\ref{eq:gcl}), the scaled physical derivative $J^k\pd{u}{x_i}$ can be computed via
\begin{equation}
J^k\pd{u}{x_i} = \frac{1}{2}\sum_{j=1}^d \LRp{J^k\bm{G}^k_{ij}\pd{u}{\hat{x}_j} + \pd{\LRp{J^k\bm{G}^k_{ji}u}}{\hat{x}_j}}.
\label{eq:splitderiv}
\end{equation}

We define physical differentiation matrices based on the approximation of (\ref{eq:splitderiv}).  Define $\bm{D}^i_k$ as 
\[
\bm{D}^i_k = \sum_{j=1}^d {\rm diag}\LRp{\bm{JG}^k_{ij}}\bm{D}^j_N + \bm{D}^j_N{\rm diag}\LRp{\bm{JG}^k_{ji}}
\]
...
\begin{equation}
\bm{D}^i_k = \sum_{j=1}^d \LRp{\bm{D}^j_N \circ \avg{\bm{JG}^k_{ji}}}, \qquad \avg{\bm{JG}^k_{ji}}_{mn} = \frac{1}{2}\LRp{\LRp{\bm{JG}^k_{ji}}_m + \LRp{\bm{JG}^k_{ji}}_n},
\label{eq:dik}
\end{equation}
where $\avg{\bm{JG}^k_{ji}}$ denotes the matrix of averages between each of the entries of $\bm{JG}^k_{ij}$.  From (\ref{eq:dik}), it is straightforward to show that $\bm{Q}^i_k = \bm{W}_N\bm{D}^i_k$ also satisfies a summation by parts property
\begin{equation}
...
\[
\sum_{j=1}^d J\bm{G}_{ij} \hat{n}_i\hat{J}_f = n_i J^k_f, \qquad \LRp{\sum_{j=1}^d \LRp{\bm{JG}^k_{ji}} \circ\LRp{\hat{\bm{n}}_i\hat{\bm{J}}_f}}
...
\begin{theorem}
Assume that $\bm{Q}^j_k\bm{1} = 0$ for $j = 1,\ldots,d$.  Then, (\ref{eq:dgform1}) is entropy conservative in the sense that
\[
...
\end{theorem}
\begin{proof}
Using that $bm{Q}^j_k\bm{1} = 0$ and (\ref{eq:sbpk}), the proof of entropy conservation is identical to that of \cite{chan2017discretely}.
...
\note{Note that this is equivalent to free-stream preservation!}
