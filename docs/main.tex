...
J^k\bm{G}^k_{i1}\\
J^k\bm{G}^k_{i2}\\
J^k\bm{G}^k_{i3}\end{array}} &\approx I_N\LRp{\hat{\Grad}\times I_{N+1}\LRp{\pd{\bm{x}}{\hat{x}_j}\times \pd{\bm{x}}{\hat{x}_k}}}, \qquad i,j,k \text{ cyclic}.
\label{eq:iconscurl2}
\end{align}
...
%\bm{V}_f
%\end{array}}
%\bm{P}_q\tilde{\bm{JG}^k_{ij}} = \tilde{\bm{JG}^k_{ij}}
%\]
...
\]
by the construction using (\ref{eq:iconscurl2}), $\sum_{j=1,\ldots,d} \bm{V}_q\bm{D}_i\tilde{\bm{JG}^k_{ij}} = \bm{0}$ as well.

Equation (\ref{eq:normalconsistency}) of Assumption~\ref{ass:norm} is satisfied by directly constructing the scaled normals using values of $\tilde{J^k\bm{G}^k_{ij}}$ at quadrature points.  Equation (\ref{eq:normalsign}) is \note{not proven yet, but I do observe it!}.  The accuracy of the approximation is a result \note{that I still need to show, but it checks out numerically too.}
