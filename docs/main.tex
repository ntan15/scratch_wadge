...
\[
S(\bm{u}) - S(\bm{u}_v) \geq \pd{S}{\bm{u}}(\bm{u}_v)^T (\bm{u}-\bm{u}_v) = \Pi_N\bm{v}(\bm{u})^T (\bm{u}-\bm{u}_v).  
\]
We can examine the components of the RHS: if the mathematical entropy increases (i.e. $S(\bm{u}_v) > S(\bm{u})$, then some components will be negative, indicating what $\Theta$ should be.  \note{Todo: figure out proper scaling the diagonal elements of $\Theta$.}
