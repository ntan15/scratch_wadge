...
\label{eq:dgcl}
\end{equation}
This condition is also related to free-stream preservation, which ensures (at the discrete level) that for $\bm{u}$ constant, 
\[
...
\note{Clarify isoparametric here - use degree $N$ polynomials to approximate, so that second block row of $\bm{Q}_N$ is automatically zero.}
...
\subsection{A discretely entropy stable weight-adjusted DG formulation on curved meshes}
...
&\bm{f}^* = \bm{f}_S(\tilde{\bm{u}}_f^+,\tilde{\bm{u}}_f) \text{ on interior faces}.\nonumber
\end{align}
We can show that formulation (\ref{eq:dgform2}) is entropy conservative in the same sense as Theorem~\ref{thm:stab2}: 
\begin{theorem}
...
\]
where $\bm{M}$ is the $L^2$ mass matrix over $D^k$ and $\bm{K}_N$ is the Gram matrix corresponding to the Sobolev inner product for $W^{N+1,2}\LRp{D^k}$.  We emphasize, however, that the constant $C_N$ depends on the largest eigenvalue, which in turn depends on the order $N$ and dimension $d$.  
\end{proof}
...
\[
\nor{\Pi_N^k u - P_N^k  u}_{L^2\LRp{D^k}} \leq C\nor{J}_{L^{\infty}\LRp{D^k}}  \nor{\frac{1}{J}}_{L^{\infty}\LRp{D^k}}^2 \nor{J}^2_{W^{N+1,\infty}\LRp{D^k}} h^{N+2}\nor{u}_{W^{N+1,2}\LRp{D^k}}.
\]
