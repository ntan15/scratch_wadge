...
\caption{2D curved meshes used for testing entropy conservation, primary conservation, and accuracy.}
\label{fig:warp2d}
\end{figure}
...
Next, we test the accuracy of the proposed schemes for smooth solutions.  
...
FInally, we check that primary conservation is maintained numerically on curved meshes.  We follow \cite{friedrich2017entropy} and examine the semi-discrete evolution of the average of $\bm{u}$ over the domain $\Omega$
\begin{align}
...
\item \textbf{Polynomial approximation:} the weight-adjusted DG scheme (\ref{eq:dgform2}) utilizes the $L^2$ projection of $J^k$ onto $P^N$ in the weight-adjusted mass matrix (such that Lemma~\ref{eq:conscorrect1} holds), 
...
\end{enumerate}
All results use a Lax-Friedrichs flux, a CFL of $1/2$, $N=4$, and the curved mesh warping shown in Figure~\ref{fig:warp2d}.  Volume quadrature rules from \cite{xiao2010quadrature} which are exact for degree $2N+1$ polynomials were used, and $(N+1)$-node Gauss-Legendre quadrature rules were used on the faces.  

Figure~\ref{subfig:cons1} shows computed conservation residuals from (\ref{eq:consres}) on the curved mesh in Figure~\ref{fig:warp2d} for the discontinuous pulse initial condition (\ref{eq:discontin}).  The conservation residual oscillates around $O(10^{-6})$ for the unmodified scheme (\ref{eq:dgform2}).  Modifying the scheme using either the ``Polynomial approximation'' or ``Conservative correction'' approaches reduces the conservation error to $O(10^{-12})$.  The magnitude of the conservation error is attributed to roundoff effects.  Repeating the same experiments on a smaller mesh of $8\times 8$ curved elements reduces the conservation residual to $O(10^{-13})$, while using a larger mesh of $32\times 32$ curved elements results in a conservation residual of $O(10^{-11})$.  
