...
We assume that normals are consistent across neighboring elements.  Let $\bm{D}^k$ and $\bm{D}^{k,+}$ be adjacent elements sharing a face $f$.  Then, the scaled outward normal vectors for each element are equal and opposite such that $\bm{n}J^k_f = -\bm{n}^+J^{k,+}_f$.  
...
\subsection{Enforcing the discrete geometric conservation law}
...
This can be remedied by using an alternative form of the geometric terms, which ensures that (\ref{eq:dgcl}) is satisfied a-priori \cite{visbal2002use, kopriva2006metric}.  The geometric terms $J\bm{G}_{ij}$ can also be computed in a ``conservative curl'' form
\begin{align}
\LRs{\begin{array}{c}
J^k\bm{G}^k_{i1}\\
J^k\bm{G}^k_{i2}\\
J^k\bm{G}^k_{i3}\end{array}} &= \hat{\Grad}\times \LRp{\pd{\bm{x}}{\hat{x}_j}\times \pd{\bm{x}}{\hat{x}_k}}, \qquad i,j,k \text{ cyclic},
\label{eq:conscurl}
\end{align}
where $\hat{\Grad} \times $ denotes the curl computed with respect to the reference element coordinates.  A similar ``invariant curl form'' is also described in \cite{kopriva2006metric}.  From (\ref{eq:conscurl}), it can be observed that, because the divergence of a curl vanishes, the continuous GCL condition (\ref{eq:gcl}) holds
\begin{align}
...
J^k\bm{G}^k_{i1}\\
J^k\bm{G}^k_{i2}\\
J^k\bm{G}^k_{i3}\end{array}} &\approx \hat{\Grad}\times I_N\LRp{\pd{\bm{x}}{\hat{x}_j}\times \pd{\bm{x}}{\hat{x}_k}}, \qquad i,j,k \text{  cyclic},
\label{eq:iconscurl}
\end{align}
...
We adopt a slight modification of (\ref{eq:iconscurl}) in this work, approximating geometric terms by using the interpolation operator $I_{N+1}$ onto degree $(N+1)$ polynomials, then interpolating back to degree $N$ polynomials in the end
\begin{align}
\LRs{\begin{array}{c}
J^k\bm{G}^k_{i1}\\
J^k\bm{G}^k_{i2}\\
J^k\bm{G}^k_{i3}\end{array}} &\approx {\hat{\Grad}\times I_{N+1}\LRp{\pd{\bm{x}}{\hat{x}_j}\times \pd{\bm{x}}{\hat{x}_k}}}, \qquad i,j,k \text{ cyclic}.
\label{eq:iconscurl2}
\end{align}
We note that, since the curl of a degree $(N+1)$ polynomial is degree $N$, the approximation generated by (\ref{eq:iconscurl2}) is a degree $N$ polynomial.  
\begin{theorem}
Let the geometric terms $\tilde{J^k\bm{G}^k_{ij}}$ denote the approximation of $J^k\bm{G}^k_{ij}$ using (\ref{eq:iconscurl2}), and let the scaled normals $\tilde{n_iJ^k_f}$ be computed using $\tilde{J^k\bm{G}^k_{ij}}$ and (\ref{eq:normalconsistency}).  Then, $\tilde{J^k\bm{G}^k_{ij}}$ is an $O(h^{N+1})$ accurate approximation which satisfies both the discrete GCL condition (\ref{eq:dgcl}) and Assumption~\ref{ass:norm}.
\end{theorem}
\begin{proof}
The satisfaction of (\ref{eq:dgcl}) relies on the fact that $J^k\bm{G}^k_{ij}\in P^N$, and that $J^kG^k_{ij}$ is equal to its own $L^2$ projection.  Then, 
\[
\LRs{
\begin{array}{c}
\bm{V}_q\\
\bm{V}_f
\end{array}}\bm{P}_q\bm{JG}^k_{ij} = \bm{JG}^k_{ij}
\]
by the fact that the discrete $L^2$ projection operator applied to $\bm{JG}^k_{ij}$ returns its exact polynomial coefficients.  
\note{Finish}
\end{proof}

\begin{remark}
We note that the accuracy of (\ref{eq:iconscurl2}) depends on the choice of interpolation points.  It is well known that interpolating at equispaced points can result in inaccurate polynomial approximations.  One can determine good interpolation point sets by optimizing over some measure of interpolation quality (such as the Lebesgue constant), and in practice, sets of interpolation points are pre-computed for some polynomial degrees $N = 1,\ldots, N_{\max}$ on the reference element and stored \cite{chen1996optimal, hesthaven1998electrostatics}. Interpolation points can also be explicitly computed as the image of equispaced points under an appropriately defined mapping
\cite{blyth2006lobatto, warburton2006explicit, chan2015comparison}.  
\end{remark}
...
%\note{Explain that WADG is not strictly locally conservative with respect to $J$, show how to perform a high order correction.  Mention that in numerical experiments, results with and without conservation correction are essentially indistinguishable from each other.}
...
%
%We introduce a ``reference'' global DG derivative, which is defined (for $v\in P^N$ but $u,w\not\in P^N$)
%\begin{align*}
...
%\begin{theorem}
%\label{thm:ec_curvi}
%Let $\bm{u}_{\bm{v}} = \bm{u}\LRp{P_N^k \bm{v}(\bm{u})}$.  Then, the semi-discrete DG discretization 
%\[
...
We begin by verifying Theorem~\ref{thm:superconverge}.  Figure~\ref{fig:superconverge}
...
\subfloat[$f(\bm{x}) = H(x+y-\sin(\pi x))$]{
\begin{tikzpicture}
\begin{loglogaxis}[
    legend cell align=left,
    legend style={legend pos=south east, font=\tiny},
...
\addplot[color=blue,mark=*,semithick, mark options={solid,fill=markercolor}]
coordinates{(0.5,0.323047)(0.25,0.22788)(0.125,0.160388)(0.0625,0.114753)(0.03125,0.0792918)};
\logLogSlopeTriangleFlip{0.4}{0.25}{0.875}{.5}{blue}
\addplot[color=red,mark=x,dashed,semithick, mark options={solid,fill=markercolor}]
coordinates{(0.5,0.323882)(0.25,0.227986)(0.125,0.160412)(0.0625,0.114756)(0.03125,0.0792924)};
\addplot[color=black,mark=triangle*,semithick, mark options={solid,fill=markercolor}]
coordinates{(0.5,0.00428104)(0.25,0.00080429)(0.125,0.000182915)(0.0625,2.96705e-05)(0.03125,5.01885e-06)};
...
\caption{$L^2$ errors in approximating both smooth and discontinuous functions $f(\bm{x})$ using $L^2$ and weight-adjusted projections on a curved mesh.  The approximation order is $N=4$, and a degree $2N$ quadrature rule is used to compute integrals over the reference triangle. }
