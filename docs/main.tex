...
We next define the $L^2$ norm and inner products over the reference element $\hat{D}$ and the surface of the reference element $\partial \hat{D}$
\[
...
\end{align*}
The $L^2, L^{\infty}$ Sobolev norms of degree $s$ are then defined 
\begin{align*}
...
\]
The Sobolev spaces $W^{s,2},W^{s,\infty}$ are defined as the spaces of functions with finite $L^2$ and $L^\infty$ Sobolev norms of degree $s$.
...
\begin{align}
\sum_{j=1}^d J^k\bm{G}^k_{ij} \hat{n}_j\hat{J}_f = n_i J^k_f, \qquad \LRp{\sum_{j=1}^d \LRp{\bm{JG}^k_{ij}} \circ\LRp{\hat{\bm{n}}_j\hat{\bm{J}}_f}}
...
Next, we verify the semi-discrete entropy conservation and accuracy of the proposed high order methods for the compressible Euler equations on curved meshes in two and three dimensions.  We follow \cite{chan2015gpu} and choose the timestep $dt$ as
\begin{equation}
...
\begin{equation}
\text{entropy RHS}= \sum_{j=1}^d\LRp{ \tilde{\bm{v}}^T\LRp{\bm{Q}^j_k \circ \bm{F}_S}\bm{1} + \tilde{\bm{v}}_f^T\bm{W}_f^k {\rm diag}\LRp{\bm{n}_i}\LRp{\bm{f}^* - \bm{f}(\tilde{\bm{u}}_f)}}.  
\label{eq:entropyrhs}
\end{equation}
