...
{
  void *opts;
  {/* comment to help vim folding */ {{const char *const *arg_p = argv + 1;
  size_t opt_count = 1;
  for (; *arg_p; ++arg_p)
  {
    if ('-' == (*arg_p)[0] && (*arg_p)[1])
    {
      ...
      else
      {
        ...
        for (; opt_spec_p->key; ++opt_spec_p)
        {
          ...
          {
            opt_count +=
                opt_spec_p->flags & ASD_GOPT_ARG ? 1 : strlen((*arg_p) + 1);
            break;
          }
        }
      }
  ...
  opts = asd_malloc(opt_count * sizeof(opt_t));
}
}
}
{
        ...
        {
          {
            ...
            found_long:
              ...
              if (!(opt_spec_p->flags & ASD_GOPT_REPEAT))
              {
                const opt_t *opt_p = opts;
                for (; opt_p != next_option; ++opt_p)
                  if (opt_p->key == opt_spec_p->key)
                  {
                    ASD_LERROR("%s: --%.*s: option may not be repeated (in "
              ...
              if (opt_spec_p->flags & ASD_GOPT_ARG)
              {
                ...
                if ((char *)0 + 1 == next_option->arg)
                {
                  ...
                  {
                    ASD_LERROR("%s: --%s: option requires an option argument\n",
                               argv[0], (*(arg_p - 1)) + 2);
                    asd_free(opts);
                    exit(EX_USAGE);
                  }
                ...
                if (strchr(opt_spec_p->shorts, *short_opt))
                {
                  if (!(opt_spec_p->flags & ASD_GOPT_REPEAT))
                  {
                    ...
                    for (; opt_p != next_option; ++opt_p)
                      if (opt_p->key == opt_spec_p->key)
                      {
                        ASD_LERROR("%s: -%c: option may not be repeated (in "
                                   "any long or short form)\n",
                                   argv[0], *short_opt);
                        asd_free(opts);
                        exit(EX_USAGE);
...
size_t asd_gopt(const void *vptr_opts, int key)
{
...
size_t asd_gopt_arg(const void *vptr_opts, int key, const char **arg)
{
  const opt_t *opts = vptr_opts;
  size_t count = 0;
...
const char *asd_gopt_arg_i(const void *vptr_opts, int key, size_t i)
{
  const opt_t *opts = vptr_opts;
...
size_t asd_gopt_args(const void *vptr_opts, int key, const char **args,
                     size_t args_len)
{
  ...
  for (; opts->key; ++opts)
    if (opts->key == key)
    {
      if (args_stop == args_ptr)
        return args_len + asd_gopt(opts, key);
...
void asd_gopt_free(void *vptr_opts) { asd_free(vptr_opts); }
...
int asd_critbit0_contains(asd_critbit0_tree_t *t, const char *u)
{
  ...
  while (1 & (intptr_t)p)
  {
    asd_critbit0_node_t *q = (void *)(p - 1);
                                ...
                                int (*handle)(const char *, void *), void *arg)
{
  ...
  if (1 & (intptr_t)top)
  {
    asd_critbit0_node_t *q = (void *)(top - 1);
...
int asd_dictionary_contains(asd_dictionary_t *d, const char *key)
{
  ...
  char *u = asd_malloc(sizeof(char) * (keylen + 2));
...
int asd_dictionary_insert(asd_dictionary_t *d, const char *key, const char *val)
{
  ...
  char *keyval = asd_malloc(sizeof(char) * (keylen + vallen + 2));
...
int asd_get_host_rank(MPI_Comm comm)
{
  ...
  int *host_ranks = asd_malloc(size * sizeof(int));
  char *allnames = asd_malloc(size * MPI_MAX_PROCESSOR_NAME * sizeof(char));
...
// {{{ linear algebra
static void asd_la_mtranspose(size_t m, size_t n, long double *restrict A,
                              size_t lda, long double *restrict B, size_t ldb)
{
...
static void asd_la_mTmmult(size_t m, size_t n, size_t k,
                           long double *restrict A, size_t lda,
                           long double *restrict B, size_t ldb,
                           long double *restrict C, size_t ldc)
{
  for (size_t i = 0; i < m; ++i)
    for (size_t j = 0; j < n; ++j)
...
static void asd_la_mmmult(size_t m, size_t n, size_t k, long double *restrict A,
                          size_t lda, long double *restrict B, size_t ldb,
                          long double *restrict C, size_t ldc)
{
  for (size_t i = 0; i < m; ++i)
...
static void asd_la_lu_factor(size_t n, long double *restrict A,
...
static void asd_la_lu_solve(size_t n, long double *restrict LU,
                            long double *restrict x, size_t *restrict p,
                            size_t *restrict q, long double *restrict work)
{
  /*
   * Compute $Pb$
   */
...
static void asd_la_backslash(size_t m, size_t n, long double *restrict A,
                             long double *restrict B, long double *restrict C)
{
  long double *LU = asd_malloc_aligned(m * m * sizeof(long double));
  long double *work = asd_malloc_aligned(m * sizeof(long double));

  size_t *p = asd_malloc_aligned(m * sizeof(size_t));
  size_t *q = asd_malloc_aligned(m * sizeof(size_t));
...
static void asd_la_forwardslash(size_t m, size_t n, long double *restrict A,
                                long double *restrict B,
                                long double *restrict C)
{
  long double *AT = asd_malloc_aligned(n * m * sizeof(long double));
  long double *BT = asd_malloc_aligned(n * n * sizeof(long double));
  long double *CT = asd_malloc_aligned(n * m * sizeof(long double));
                                             ...
                                             long double *restrict w)
{
  ...
  long double *restrict theta0 = asd_malloc_aligned(nk * sizeof(long double));
  long double *restrict theta1 = asd_malloc_aligned(nk * sizeof(long double));
  long double *restrict p0 = asd_malloc_aligned(nk * sizeof(long double));
  long double *restrict dp0 = asd_malloc_aligned(nk * sizeof(long double));
                                ...
                                long double *I)
{
  ...
  long double *Vx = asd_malloc_aligned((N + 1) * nx * sizeof(long double));
...
{
  long double *I = asd_malloc_aligned((N + 1) * (N + 1) * sizeof(long double));

  long double *invV =
      asd_malloc_aligned((N + 1) * (N + 1) * sizeof(long double));
